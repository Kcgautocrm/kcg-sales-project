...
export async function POST(request: Request) {
  try {
    ...
    let {isAuthorized} = await authService(token, ["salesPerson", "supervisor"])
    if(!isAuthorized){
    ...
    return new NextResponse(JSON.stringify({ message: `This service has been suspended`, data: null }), { 
     status: 201, 
     headers: { "Content-Type": "application/json" },
    });
  } catch (error: any) {
